...
impl ViewItem {
    ...
    fn new(main_state: &MainState) -> Self {
        ...
        let scrolled_window = builder.get_object("scrolled_window").unwrap();
    ...
    fn connect_events(&self, edit_view: &Rc<RefCell<EditView>>) {
        ...
        self.scrolled_window.connect_realize(|w| {
            // Set the text cursor
        ...
        self.scrolled_window
            .connect_scroll_event(enclose!((edit_view) move |_,es| {
                edit_view.borrow().handle_scroll(es)
            }));
...
impl EditView {
    ...
    pub fn handle_scroll(&self, es: &EventScroll) -> Inhibit {
        ...
        self.view_item.scrolled_window.set_vadjustment(vadj);
        self.view_item.scrolled_window.set_hadjustment(hadj);
        self.update_visible_scroll_region();
